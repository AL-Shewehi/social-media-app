...
export default function CommentsDialog({ trigger, post }: CommentsDialogProps) {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="flex gap-3 bg-secondary/30 p-3 rounded-lg border border-border/50">
            <Avatar className="h-9 w-9 border">
              <AvatarFallback className="bg-primary text-white font-bold text-xs">{fallbackLetter}</AvatarFallback>
            </Avatar>
            <div className="flex-1">
              <h5 className="font-semibold text-sm">{authorName}</h5>
              <p className="text-[14px] text-foreground mt-1 wrap-break-word" dir="auto">{post.content}</p>
            </div>
          </div>
