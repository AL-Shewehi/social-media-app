...
import type { PostCardPost, Profile, Comment } from "@/types/database.types";
...
export default function PostDetailInteractions({
  ...
  const { data: comments = [] } = useQuery({
    ...
    queryFn: () =>
      fetchPostCommentsAction(post.id).then((r) => {
  ...
  const likeMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const previous = queryClient.getQueryData<PostCardPost>(
        ...
        .forEach((q) => {
          queryClient.setQueryData<InfiniteData<PostCardPost[]>>(
            ...
            (old) => {
              ...
              return {
                ...old,
                pages: old.pages.map((p) => p.filter((x) => x.id !== post.id)),
              };
            },
          );
        });
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col bg-card">
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
              ...
              <div className="bg-secondary/50 rounded-xl p-3 border">
                ...
                {post.shared_post.content && (
                  <p className="text-xs text-muted-foreground mb-2">
                    {linkifyText(post.shared_post.content)}
                  </p>
                )}
          ...
          <div className="flex items-center justify-between px-4 py-2 border-b text-sm text-muted-foreground shrink-0">
            ...
            >
              <ThumbsUp
                className={cn(
                  "h-4 w-4",
                  displayIsLiked && "fill-primary text-primary",
                )}
              />
          ...
          <div className="flex items-center border-b shrink-0">
            ...
            >
              <ThumbsUp
                className={cn("h-5 w-5", displayIsLiked && "fill-primary")}
              />
              إعجاب
            </Button>
